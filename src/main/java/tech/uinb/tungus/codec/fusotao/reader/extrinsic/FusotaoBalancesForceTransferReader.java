...
public class FusotaoBalancesForceTransferReader implements ScaleReader<FusotaoBalancesForceTransfer> {
	@Override
	public FusotaoBalancesForceTransfer read(ScaleCodecReader rdr) {
		var result = new FusotaoBalancesForceTransfer();
		result.setSource(new StaticLookupSourceReader(SS58Type.Network.SUBSTRATE).read(rdr));
